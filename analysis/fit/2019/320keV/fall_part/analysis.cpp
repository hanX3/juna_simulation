...
//rise part
double risepartstart = 2203.;
double risepartstop = 2233.;
...
void analysis(double proton_energy)
{
  ...
  std::ofstream ofs;
  ofs.open ("../../../../../../result/2019/320keV/chi2_fall_part.dat", std::ofstream::out | std::ofstream::app);

  //data path
  char filename[1024];
  char savepdf[1024];
  sprintf(filename, "../../../../../../rootfile/2019/proton_%0.1fkeV.root", proton_energy);
  sprintf(savepdf, "../../../../../../result/2019/320keV/fall_part_%0.1f.pdf", proton_energy);
  ...
  grexp->GetYaxis()->SetRangeUser(0., 1200.);
  ...
  gtemp->GetYaxis()->SetRangeUser(0., 1200.);
  gtemp->Draw("AP");
  grsimulationscale->Draw("P same");
  tffallp->Draw("same");
  tfbg->Draw("same");
